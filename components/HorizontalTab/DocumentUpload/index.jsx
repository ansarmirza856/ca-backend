...
const index = ({ data }) => {
  ...
  //       `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/get-single-application?id=${formId}`
  //     );
  ...
  const uploadFile = async (file) => {
    try {
      const result = await Storage.put(file.originalFilename, file, {
        level: "public",
      ...
      const awsUrl = url.substring(0, url.indexOf("?"));
      return awsUrl;
    } catch (error) {
